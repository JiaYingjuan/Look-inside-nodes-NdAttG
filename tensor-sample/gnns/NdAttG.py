...
def sparse_NdAttG_layer(node_embeddings: tf.Tensor,
    ...
    In the setting for a single attention head, we compute new states as follows:
        \phi =h^t_v-\bar{h^t_v} 
        \mathcal{\psi} =\phi \circ \phi
        \Phi= \frac{\mathcal{\psi}}{\bar{\mathcal{\psi}}+\varepsilon}
        \vartheta= \sigma(\omega \cdot \Phi + \lambda)
        h^t_v = \vartheta \circ h^t_v
    ...
    * E: number of edges of a given edge type
    ...
    h = node_embeddings
    # Define trainable omega and lambda for LPAT, and initialize varepsilon.
    omega = tf.Variable(initial_value=1.0, trainable=True, name='omega')
    lambd = tf.Variable(initial_value=5.0, trainable=True, name='lambd')  # initial_value=5.0 for QM9;
    varepsilon= 1e-4
    h_mean = tf.reduce_mean(h, axis=1, keepdims=True) 
    Psi = tf.pow(h - h_mean, 2) 
    Psi_mean = tf.reduce_mean(Psi, axis=[1], keepdims=True)
    vartheta = tf.sigmoid(omega * Psi / (Psi_mean+ varepsilon) + lambd)
    h_vartheta = h * vartheta
    cur_node_states = h_vartheta
